...
@interface BLScanView ()<AVCaptureMetadataOutputObjectsDelegate>
{
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    if (metadataObjects.count>0) {
        ...
        if (data) {
            ...
            if(_delegate && [_delegate respondsToSelector:@selector(getScanDataString:)])
            {
                [_delegate getScanDataString:scanMessage];
            }
            NSLog(@"%@",scanMessage);
        }
    }
...
- (void)setOverlayPickerView
{
    ...
    //扫描框图片的拉伸，拉伸中间一块区域
    UIImage *scanImage = [UIImage imageNamed:@"blScan"];
...
- (void)addAnimation{
    ...
    line.hidden = NO;
    CABasicAnimation *animation = [BLScanView moveYTime:2 fromY:[NSNumber numberWithFloat:4] toY:[NSNumber numberWithFloat:self.scanW -2] rep:OPEN_MAX];
